...
const Navbar = () => {
  ...
  return (
    ...
    >
          ...
          {navItems.map((item) => (
            <li key={item.href}>
              <a
                ...
                className="text-sm text-muted-foreground hover:text-primary transition-colors px-3 py-2 rounded-md hover:bg-primary/5"
              >
                {item.label}
              </a>
            </li>
      ...
      {/* Mobile menu */}
      {mobileOpen && (
        <div className="md:hidden bg-background/95 backdrop-blur-md border-b border-border/40 px-4 pb-4">
          {navItems.map((item) => (
            <a
              key={item.href}
              href={item.href}
